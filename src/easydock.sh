...
if [ $1 == "export" ]; then
    ...
    ed_logo
    unlink ./.env.easydock
    cp ./vendor/andreapollastri/easydock/src/.env.easydock ./.env.easydock
    unlink ./docker-compose.yml
    cp ./vendor/andreapollastri/easydock/src/docker-compose.yml ./docker-compose.yml
    unlink ./ed
    cp ./vendor/andreapollastri/easydock/src/.easydock.sh ./ed
    sed -i -e 's/APP_SID=easydock/APP_SID=easydock'$(((RANDOM%1000000000)+1))'/g' .env.easydock
    unlink ./.env.easydock-e
    clear
    ed_logo
    echo "${bgpurple}${white} easydock is ready! All you have to do is compile your ${bold}.env.easydock${reset}${bgpurple}${white} file and run ${bold}sh ed setup${reset}${bgpurple}${white} ;) ${br}${reset}"
    exit
fi
...
if [ $1 == "env" ]; then
    ...
    if test -f "./.env.easydock"; then
        echo "${bgyellow}${black}${bold} >>> WARNING! ${reset} an .env.easydock file is located in your workspace. Delete it if you want regenerate it within this command. ${reset}${br}"
    else
        cp ./vendor/andreapollastri/easydock/src/.env.easydock ./.env.easydock
        sed -i -e 's/APP_SID=easydock/APP_SID=easydock'$(((RANDOM%1000000000)+1))'/g' .env.easydock
        unlink ./.env.easydock-e
        clear
        ed_logo
        echo "${bgpurple}${white} Your ${bold}.env.easydock${reset}${bgpurple}${white} has been created ;) ${br}${reset}"
    fi
    exit
fi
...
if test -f "./.env.easydock"; then
    ...
    clear
    ed_logo
    echo "${bgwhite}${red}${bold} >>> ERROR! ${reset} File ${bgred}${white}${bold} .env.easydock ${reset} is missing. Run ${bgred}${white}${bold} sh ed env ${reset} to generate it. ${reset}${br}"
    exit
fi
...
ed_reset() {
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        ed_logo
        rm -rf ./easydock/data/*
        cp ./easydock/src/.gitignore ./easydock/data/.gitignore
        ed_logo
